...
import {
  ...
  TextField } from
'@infinibay/harbor';
import { Fingerprint, RefreshCw, Shield, UserRoundCog, Users } from 'lucide-react';
...
export default function IdentityListPage() {
  ...
  const connectorInput = () => ({
    ...
    port: Number(connectorForm.port || (connectorForm.useTls ? 636 : 389)),
    useTls: connectorForm.useTls,
    baseDn: connectorForm.baseDn.trim(),
  ...
  const handleCreateConnector = async () => {
    try {
      ...
      if (!input.name || !input.host || !input.baseDn) {
        ...
        <Dialog open onClose={() => setDialogOpen(false)}>
          ...
          <DialogBody>
            <div className="flex flex-col gap-4 min-w-[520px]">
              <ResponsiveStack direction={{ base: 'col', sm: 'row' }} gap={3}>
                <Select
                  label="Type"
