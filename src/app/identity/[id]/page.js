...
export default function IdentityDetailPage() {
  ...
  const {
    ...
    skip: connectorId === 'local' || Boolean(idp),
    fetchPolicy: 'cache-and-network'
  });
  const {
    data: mappingData,
            ...
            <ResponsiveStack direction={{ base: 'col', lg: 'row' }} gap={4} align="stretch">
              <div className="flex-1 min-w-0 rounded-md border border-border-subtle bg-surface-raised p-4">
                <ResponsiveStack direction="col" gap={3}>
                  ...
                  <KV label="Last test" value={provider.lastTestAt ? timeAgo(provider.lastTestAt) : '-'} />
                  <KV label="Last sync" value={provider.lastSyncAt ? timeAgo(provider.lastSyncAt) : '-'} />
  ...
  if (['active-directory', 'ldap', 'idp-acme-ad', 'idp-okta'].includes(connectorId)) {
    const connectorName =
      connectorId === 'ldap' ? 'LDAP' :
      connectorId === 'idp-okta' ? 'Okta' :
      'Active Directory';
      ...
      <Page>
        <ResponsiveStack direction="col" gap={4}>
          <PageHeader
            title={connectorName}
            count="not connected"
            description="External directory sync is not wired to the backend yet."
            secondary={
            <Button
              size="sm"
              variant="secondary"
          ...
          <Alert
            ...
            title="Connector backend required">
              This page intentionally avoids mock directory users, mappings, and sync runs. The backend needs connector support before operators can test, sync, or map this source.
            </Alert>
          ...
          <ResponsiveStack
            ...
            <div className="flex-1 min-w-0 rounded-md border border-border-subtle bg-surface-raised p-4">
              <ResponsiveStack direction="col" gap={3}>
                ...
                <ResponsiveStack direction="row" gap={2} align="center">
                  <StatusDot status="offline" size={8} />
                  <span className="text-sm text-fg-muted">
                    No server profile, credentials, sync schedule, or group mapping exists for this connector.
                  </span>
                </ResponsiveStack>
              </ResponsiveStack>
            </div>
            ...
            <div className="w-full lg:w-72 rounded-md border border-border-subtle bg-surface-raised p-4">
              <ResponsiveStack direction="col" gap={1}>
                ...
                <span className="text-sm text-fg-muted">synced users</span>
              </ResponsiveStack>
            </div>
          </ResponsiveStack>
        </ResponsiveStack>
