...
export default function IdentityDetailPage() {
  ...
  const mappingColumns = useMemo(() => [
    { key: 'group', label: 'Source group', render: (r) => <span className="font-mono text-xs">{r.group}</span> },
    { key: 'type', label: 'Type', width: 130, render: (r) => <Badge tone="neutral">{r.type}</Badge> },
    { key: 'maps', label: 'Maps to', render: (r) => <span>{r.maps}</span> },
  ], []);
  ...
  const userColumns = useMemo(() => [
    ...
    { key: 'samAccountName', label: 'Login', width: 180, render: (r) => <span className="font-mono text-xs">{r.samAccountName}</span> },
    { key: 'email', label: 'Email', render: (r) => <span className="text-sm">{r.email}</span> },
    { key: 'lastLoginAt', label: 'Last login', width: 120, render: (r) => <span className="text-sm">{timeAgo(r.lastLoginAt)}</span> },
  ], []);
  ...
  const runColumns = useMemo(() => [
    ...
    { key: 'startedAt', label: 'Started', width: 150, render: (r) => <span className="text-sm">{timeAgo(r.startedAt)}</span> },
    { key: 'durationMs', label: 'Duration', width: 100, render: (r) => <span className="font-mono text-xs">{r.durationMs} ms</span> },
    { key: 'usersAdded', label: '+', width: 60, align: 'right', render: (r) => <span className="font-mono text-xs">{r.usersAdded}</span> },
    { key: 'usersUpdated', label: '~', width: 60, align: 'right', render: (r) => <span className="font-mono text-xs">{r.usersUpdated}</span> },
    { key: 'usersRemoved', label: '−', width: 60, align: 'right', render: (r) => <span className="font-mono text-xs">{r.usersRemoved}</span> },
    { key: 'errors', label: 'Errors', render: (r) => r.errors?.length ? <span className="text-sm text-danger">{r.errors[0]}</span> : <span className="text-fg-subtle">—</span> },
  ], []);
      ...
      <ResponsiveStack direction="col" gap={4}>
        ...
        {tab === 'sync' && (
          <section className="flex flex-col gap-2">
            ...
            <DataTable rows={runs} columns={runColumns} rowKey={(r) => r.id} dense />
          </section>
        )}
        ...
        {tab === 'mapping' && (
          <section className="flex flex-col gap-2">
            ...
            <DataTable rows={mappings} columns={mappingColumns} rowKey={(r) => r.id} dense />
          </section>
        )}
