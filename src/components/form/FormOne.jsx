...
import { CREATE_MACHINE_MUTATION } from "@/graphql/queries";
...
export default function FormOne() {
  ...
  const [createMachine, { loading, error }] = useMutation(CREATE_MACHINE_MUTATION);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const osVersion = selectedOS === "windows" ? selectedWindows : selectedLinux;
  ...
  const renderStep = () => {
    switch (step) {
          ...
          <div className="col-span-1 sm:col-span-3 w-[96%]">
            <div className="relative bg-white rounded-2xl border border-web_borderGray mx-auto">
              ...
              <div className="absolute top-1 translate-y-1/2 flex flex-row items-center gap-3 right-4 my-2">
                <p className={`capitalize text-xs 4xl:text-xl hidden md:block ${deviceName ? "text-web_green" : "text-web_placeHolder"}`}>
                  {deviceName ? "Name Created" : "Create Name"}
                </p>
                <Image
                  alt=""
                  width={1000}
                  height={1000}
                  src={deviceName ? "/images/form/greeenCloud.svg" : "/images/form/redCloud.svg"}
      ...
      case 2:
        return (
          <div className="col-span-1 sm:col-span-3 w-[96%]">
            <div className={`forWindows 4xl:my-16 w-full mt-10 grid place-items-center lg:grid-cols-2 grid-cols-1 gap-5 ${selectedOS === "windows" ? "grid" : "hidden"}`}>
              <div
                className={`border rounded-3xl shadow-sm p-10 4xl:px-16 4xl:py-24 w-full cursor-pointer
                ${selectedWindows === "Windows10" ? "border-web_green border-dashed" : ""}`}
                onClick={() => handleWindowsChange("Windows10")}
              >
                <div
                  className={`shadow-md h-8 w-8 4xl:h-10 4xl:w-10 border rounded-full flex items-center justify-center ml-auto ${selectedWindows === "Windows10" ? "bg-green-500" : ""
                    }`}
                >
              ...
              <div
                className={`border rounded-3xl shadow-sm p-10 4xl:p-16 4xl:py-24 w-full cursor-pointer
                ${selectedWindows === "Windows11" ? "border-web_green border-dashed" : ""}`}
                onClick={() => handleWindowsChange("Windows11")}
              >
                <div
                  className={`shadow-md h-8 w-8 4xl:h-10 4xl:w-10 border rounded-full flex items-center justify-center ml-auto ${selectedWindows === "Windows11" ? "bg-green-500" : ""
                    }`}
                >
            ...
            <div className={`forLinux w-full 4xl:my-16 mt-10 grid place-items-center lg:grid-cols-2 grid-cols-1 gap-5 ${selectedOS === "linux" ? "grid" : "hidden"}`}>
              <div
              ...
              >
                <div
                  className={`shadow-md h-8 w-8 4xl:h-10 4xl:w-10 border rounded-full flex items-center justify-center ml-auto ${selectedLinux === "redhat" ? "bg-green-500" : ""
                    }`}
                >
              ...
              >
                <div
                  className={`shadow-md h-8 w-8 4xl:h-10 4xl:w-10 border rounded-full flex items-center justify-center ml-auto ${selectedLinux === "ubuntu" ? "bg-green-500" : ""
                    }`}
                >
          ...
          <div className="col-span-1 sm:col-span-3 w-[96%]">
            <div className="mt-10 4xl:mt:20">
              <div className="flexbox">
                <h3 className="4xl:mb:4 text-2xl 4xl:text-5xl font-bold">License</h3>
                <h3 className="text-2xl 4xl:text-3xl flex gap-2 font-bold text-[#484848] items-center">
                  No-License <span><IoCheckboxOutline className="text-3xl text-web_lightbrown" /></span>
                </h3>
              </div>
                ...
                {licenseKey.map((part, index) => (
                  <React.Fragment key={index}>
                    ...
                    <Input
                      ...
                      classNames={{
                        input: "4xl:text-[28px]",
                        inputWrapper: "shadow-md 4xl:p-10 4xl:py-12 border bg-transparent",
                      }}
                    />
                  </React.Fragment>
                ))}
  ...
  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-2 sm:grid-cols-4 gap-4 md:place-content-center">
      {renderStep()}
      ...
      <div className="col-span-2 sm:col-span-4 mt-8 flex justify-between">
        {step > 1 && (
          <Button onClick={prevStep} className="bg-gray-200 text-gray-800 font-semibold py-2 px-4 rounded-full">
            Previous
          </Button>
        )}
        {step < 3 ? (
          <Button onClick={nextStep} className="btnGradientLightBlue text-white font-semibold py-2 px-4 rounded-full">
            Next
          </Button>
        ) : (
          <Button
            type="submit"
            disabled={loading}
            className="btnGradientLightBlue text-white font-semibold py-2 px-4 rounded-full"
          >
      ...
      {error && (
        <div className="col-span-2 sm:col-span-4 mt-4 text-red-500 text-center">
          Error creating machine: {error.message}
        </div>
      )}
    </form>
  );
